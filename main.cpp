    ...
    while (std::getline(file, line))
        if (line[0] == '}') currentFunction = 0;
            ...
            for (size_t i = 0; i < line.size(); i++)
                ...
                else if (line[0] != 'g' && line[i] == 'G' && !isFuncCall) isFuncCall = true;
                else if (line[i] == 'c') qubits[0]++;
                else if ((line[i] == ']' || line[i] == ' ' || line[i] == ';') && currentNum != "") {qubits.push_back(std::stoi(currentNum)); currentNum = "";}
            
            if (isFuncCall) {
                std::vector<int> newQubits = {qubits[0]};
                for (int i = 2; i < 2+qubits[0]; i++) newQubits.push_back(qubits[i]);
    ...
    #pragma omp parallel default(shared)
    {
        ...
        while (!quit) {
            ...
            for (int i = 0; i < int(circuits[currentCircuit].size()); i++) {
                auto currentCircuit_i = circuits[currentCircuit][i];
                auto currentCircuit_i_size = currentCircuit_i.size();
                int currentCircuit_i_0 = currentCircuit_i[0];
                
                if (currentCircuit_i_size == 1 && currentCircuit_i_0 == -1) #pragma omp barrier
                else if (currentCircuit_i_size >= 2 && currentCircuit_i_0 < 0) {
                    if (thread == -currentCircuit_i_0) i += currentCircuit_i[1];
                } else {
                    int currentCircuit_i_0_1 = currentCircuit_i_0+1;
                    for (int j = 1; j < currentCircuit_i_0_1; j++) if (!state[currentCircuit_i[j]]) goto notAllOn;
                    
                    if (currentCircuit_i[currentCircuit_i_0_1] == info[0]) for (size_t j = 1; j < currentCircuit_i_size; j++) state[currentCircuit_i[j]] = rand() % 2;
                    else for (size_t j = currentCircuit_i_0_1; j < currentCircuit_i_size; j++) {
                        int currentCircuit_i_j = currentCircuit_i[j];
                        if (currentCircuit_i_j < 0) {
                            prevPos.push_back({currentCircuit, i});
                            currentCircuit = -currentCircuit_i_j;
                            i = -1; break;
                        } else state[currentCircuit_i_j] = !state[currentCircuit_i_j];
                    }
                    notAllOn: // noop case
                }
            ...
            if (thread == 1) for (int i = 0; i < info[1]; i++) stateInit[i] = state[i];
            ...
            if (thread == 1) {
                ...
                for (int j = 0; j < info[4]; j++) for (int i = 0; i < info[3]; i++) if (stateInit[info[2] + info[3]*j + i]) SDL_RenderDrawPoint(renderer, i, j);
            ...
            if (thread == 1) {
                ...
                for (size_t i = 0; i < stateList.size(); stateInit[i] = keystates[stateList[i++]];);
            }
            #pragma omp barrier
        }
    }
